...
export default Ember.Route.extend(AuthenticatedRouteMixin, LoggerMixin, {
	...
	title: function() {
		...
		return i18n.t("application.title") + " | " +
			...
			i18n.t("page.admin.hotel.create-hotel.title");
	}.property(),
	...
	model(params) {
		...
		const id = params.id;
		if(id) {
			model = this.store.find('admin/hotel', id);
		} else {
			model = this.store.createRecord('admin/hotel');
		}
	...
	actions: {
		...
		/**
		 * Attempt to create a new hotel
		 */
		create() {
			const self = this;

			// Debug Message
			this.logMessage('DEBUG', 'Attempting to create a new hotel');
		...
		/**
		 * Attempt to update the hotel
		 */
		update() {
			const self = this;

			// Debug Message
			this.logMessage('DEBUG', 'Attempting to update the hotel');
